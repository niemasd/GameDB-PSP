...
# PlayStation DataCenter PSP main pages
URLS = {
    'NTSC-U': 'https://psxdatacenter.com/psp/ulist.html',
...
if __name__ == "__main__":
    ...
    for region, url in URLS.items():
        ...
        for row in soup.find_all('tr'):
            ...
            language = list()
            for lang in language_s.upper().replace('[[','][').replace('<','').replace('/TD>','').replace(';','').replace('`','').replace('J[E]','[J][E]').replace('(N(SW)','(N)(SW)').replace('{','').replace('(','').replace('}',']').replace(')',']').replace('[','').split(']'):
                if len(lang) != 0:
                    if lang not in LANG:
                        print("MISSING: %s\n%s" % (lang, cols)); exit(1)
